...
Agent: Let's create an Architecture Decision Record.

Decision title?

User: Redis for Session Cache

Agent: Context — why is this decision needed?
...
Agent: Constraints and drivers? (bullet list)
...
Agent: Options considered (name: description, pros/cons):
...
Agent: Status? (1=Proposed, 2=Accepted)

User: 1

Agent: Related ADRs? (numbers, or skip)

User: ADR-004

Agent: From a DR? (number, or skip)
...
```
User: Create an ADR from DR-008

Agent: Reading DR-008: Data Persistence...
...
```
User: Create an ADR "gRPC over REST" with status accepted

Agent: Creating ADR-014...
...
Agent: I see you decided Redis over Memcached for caching.
...
Agent: Any additional context or consequences to add?

User: Also need pub/sub for cache invalidation

Agent: Done! Created ADR-014-redis-cache-layer.md
```

## Implementation Notes Example

```
Agent: Implementation guidance? (or skip)

User: Use redis-py with connection pooling. Default TTL 30 minutes. Health check on startup.

Agent: [includes in ADR under Implementation Notes section]
```